...
if [ -z "$1" -o -z "$2" ]; then
    echo "usage: mlptest-1bank.sh <maxmlp> <corun>"
    exit 1
fi
...
outfile="mlp-onebank-bw_corun${corun}.dat"
: > "$outfile"   # truncate or create the file

for l in $(seq 1 "$mlp"); do
    for c in $(seq "$c_start" "$c_end"); do
        pll -f "$MAP_FILE" -e 0 -c "$c" -l "$l" -i 20000 -m "$memsize" >& "./pll-$l-$c.log" & # use "-m" instead of "-g" if you want to allocate memory in MB instead of GB
    done
    sleep 1.5
    pll -f "$MAP_FILE" -e 0 -c "$st" -l "$l" -i 50 -m "$memsize" 2> ./err.txt # use "-m" instead of "-g" if you want to allocate memory in MB instead of GB
